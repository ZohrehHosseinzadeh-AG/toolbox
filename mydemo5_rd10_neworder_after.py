...
if LOAD_STORED_DISTANCES == False:
    ...
    for stimid in sel_stims:
        print("Computing ISI and SPIKE distance matrix for stimid rd10_neworder_after): %d" % stimid)
        ...
        print("ISI and SPIKE complete for stimid rd10_neworder_after: %d" % stimid)
    ...
    np.save('SPIKE_dist_ys_rd10_neworder_after',SPIKE_dist_ys)
    np.save('ISI_dist_ys_rd10_neworder_after',ISI_dist_ys)
else:
    SPIKE_dist_ys = np.load('SPIKE_dist_ys_rd10_neworder_after.npy')
    ISI_dist_ys   = np.load('ISI_dist_ys_rd10_neworder_after.npy')
...
if SAVE_FIGS:
    plt.savefig('clusters_comparison_rd10_neworder_after.pdf', bbox_inches='tight')    
...
if SAVE_FIGS:
    plt.savefig('clusters_MI_rd10_neworder_after.pdf', bbox_inches='tight')    
...
if SAVE_FIGS:
    plt.savefig('clusters_distance_matrix_clustered_SPIKE_rd10_neworder_after.pdf', bbox_inches='tight')    
...
print("Distance %.2f\nNumber of (flat) clusters rd10_neworder_after: %d" % (t,n_flat_clusters))
silhouettes = metrics.silhouette_samples(distance.squareform(new_distances),fcls,metric='precomputed')
print("Mean Silhouette Coefficient rd10_neworder_after: %.2f" % np.average(silhouettes))
...
if SAVE_FIGS:
    plt.savefig('clustered_dendrogram rd10_neworder_after.pdf', bbox_inches='tight')    
...
if SAVE_FIGS:
    plt.savefig('clusters_summary_chirp_color_SPIKE_rd10_neworder_after.pdf', bbox_inches='tight')
...
if SAVE_FIGS:
    plt.savefig('clusters_tsne_chirp_SPIKE_rd10_neworder_after.pdf', bbox_inches='tight')
...
if SAVE_FIGS:
   plt.savefig('clusters_e_STA_color_mixed_rd10_neworder_after.pdf', bbox_inches='tight')
