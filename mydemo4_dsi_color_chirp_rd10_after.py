# -*- coding: utf-8 -*-
"""
Created on Wed Aug 26 17:56:08 2020
...
filenames=['2020_02_05_rd10_l1_after.mat','2020_02_05_rd10_r1_after.mat','2020_02_07_rd10_l1_after.mat','2020_02_07_rd10_r1_after.mat']
#sf=data_dir+filenames

filenames=['2020_02_05_rd10_l1_after.mat','2020_02_07_rd10_l1_after.mat']

# stimulus information
#report_filenames  = [ data_path+"2019_07_17_mrg_report.txt" ]
#trigger_filenames = [ data_path+"trigger_2019_07_17_mrg.mat" ]
report_filenames  = [ data_path+"2020_02_05_rd10_l1_after_report.txt",
##                     data_path+"2020_02_05_rd10_r1_after_report.txt",
                     data_path+"2020_02_07_rd10_l1_after_report.txt"]##,]
##                     data_path+"2020_02_07_rd10_r1_after_report.txt"]
                     


trigger_filenames = [   data_path+"2020_02_05_rd10_l1_after_trigger.mat",
##                        data_path+"2020_02_05_rd10_r1_after_trigger.mat",
                       data_path+"2020_02_07_rd10_l1_after_trigger.mat"]##,
...
#            st2.spikes=np.hstack((sts_chirp[cll][chc].spikes,sts_color[cll][chc].spikes+32000))
#
...
conditions =((~empty_trials[1]&(~empty_trials[0]))&(good_cells))
...
if LOAD_STORED_DISTANCES == False:
    ...
    np.save('SPIKE_dist_ys_rd10_after',SPIKE_dist_ys)
    np.save('ISI_dist_ys_rd10_after',ISI_dist_ys)
else:
    SPIKE_dist_ys = np.load('SPIKE_dist_ys_rd10_after.npy')
    ISI_dist_ys   = np.load('ISI_dist_ys_rd10_after.npy')
...
#t_best_chirp_color = ncls[np.argmax(metric_scores[2,m,:])]

if SAVE_FIGS:
    plt.savefig('clusters_comparison_rd10_after.pdf', bbox_inches='tight')    
...
if SAVE_FIGS:
    plt.savefig('clusters_MI_rd10.pdf', bbox_inches='tight')    
...
if SAVE_FIGS:
    plt.savefig('clusters_distance_matrix_clustered_SPIKE_rd10_after.pdf', bbox_inches='tight')    
...
if SAVE_FIGS:
    plt.savefig('clustered_dendrogram_rd10_after.pdf', bbox_inches='tight')    
...
if SAVE_FIGS:
    plt.savefig('clusters_summary_chirp_color_SPIKE_rd10_after.pdf', bbox_inches='tight')
...
if SAVE_FIGS:
    plt.savefig('clusters_tsne_chirp_SPIKE_rd10_after.pdf', bbox_inches='tight')
...
#            esta_i-=mean(esta_i)
 #           esta_i/=max(abs(esta_i))
#            high_snr[i]=abs(var(esta_i,axis=0)/(abs(np.average(esta_i,axis=0))))>.009

#            high_snr[i]=abs(var(esta_all[i][3],axis=0)/(var(esta_all[i][3],axis=0)+abs(np.average(esta_all[i][3],axis=0))))<.1
            mean_var_ratio.append(abs(np.average(esta_all[i][3],axis=0)/var(esta_all[i][3],axis=0)))
#            esta_i=np.asarray(esta_all[i][3].flatten())
#            esta_i+=800
            #min_maz[i]=abs(var(esta_i,axis=0)/(abs(np.average(esta_i,axis=0))+var(esta_i,axis=0)))
...
#            peaks, _ = find_peaks(esta_i)
#            peaks_n, _ = find_peaks(-esta_i)
...
#            peak_sta=esta_i[peaks[np.argmax(np.linalg.norm((esta_i[peaks]-mean(esta_i))))]]
...
#            peak_sta=esta_i[peaks][np.argmax(abs(esta_i[peaks]))]
            high_snr[i]=np.linalg.norm(peak_sta-mean(esta_i))>4*std(esta_ii)
            k2, p = stats.normaltest(esta_i)
             
#            mx=max(esta_i[0:half_sta])
#            mn=min(esta_i[0:half_sta])
#            mnmx=np.vstack((mx,mn)).flatten()
#
#            
#            k2, p =stats.ks_2samp(mnmx, esta_i[half_sta:])
#            s_,p=sm.stats.ztest(esta_i[0:half_sta], x2=None, value=np.average(esta_i[half_sta:]), alternative='two-sided', ddof=1.0)


            p_values[i]=p<.01
#            if high_snr[i]:
#                plt.plot(esta_i)
#                plt.plot(peaks, esta_i[peaks], "x")
#                plt.plot(np.zeros_like(esta_i)-800, "--", color="gray")
#                plt.show()
...
if SAVE_FIGS:
   plt.savefig('clusters_e_STA_color_mixed_rd10_after.pdf', bbox_inches='tight')
...
with open("clusetr_numbers_sta_rd10_after.txt", 'w') as output:
...
#    
## plot_widths = [1.2,0.8,3,0.4,0.4, 0.4, 0.4]
#plot_widths = [1,2,3,6.4,0.8, 0.4, 0.3]
#
#gs = gridspec.GridSpec(max_num_clustersx, 7, width_ratios = plot_widths, wspace=0.1, hspace=0.1)
#
#bins = 20 # ms
...
#        
#if SAVE_FIGS:
#    plt.savefig('clusters_e-STA_rd10_after.pdf', bbox_inches='tight')
