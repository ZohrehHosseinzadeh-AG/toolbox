# -*- coding: utf-8 -*-
"""
Created on Wed Sep  9 15:22:02 2020
...
# -*- coding: utf-8 -*-
"""
Created on Mon Sep  7 10:14:42 2020
...
#    ST2=[]
#    for chc in range(10):
#        st2 = spk.SpikeTrain(trial_times, 48000/(Sampling/1000))# one trial
#    #            st2.spikes=np.hstack((sts_chirp[cll][chc].spikes,sts_color[cll][chc].spikes+sts_chirp[cll][chc].spikes[-1]))
#        st2.spikes=np.hstack((sts_flash[cll][chc].spikes, sts_chirp[cll][chc].spikes+4000,sts_color[cll][chc].spikes+36000))
#    
#        st2.t_end=sts_flash[0][0].t_end+sts_chirp[0][0].t_end+sts_color[0][1].t_end
#        ST2.append(st2)     # all trials      
#    st_ch_c.append(ST2)
#    del ST2# one cell
##    if len(ST2)>0:
#plot(spk.psth(st_ch_c[3],100).y)
#plt.xlim([0,500])
#plot(spk.psth(Stimuli['SpikeTrains'][11][55],100).y)
#plt.xlim([0,40])
#plot(spk.psth(Stimuli['SpikeTrains'][0][55],100).y)
#
#Stimuli['SpikeTrains'].append(np.asarray(st_ch_c))
#from scipy.stats.stats import pearsonr
#%%
...
#    for ii,st in enumerate(Stimuli['SpikeTrains'][12][e][:]):
...
#    for ii,st in enumerate(Stimuli['SpikeTrains'][11][e][:]):
#        plt.plot(st, np.ones(len(st))+ii+i*ntrials,'k|',ms=2, lw=4)
#    plt.plot((0,44000),((i+1)*ntrials+0.5,(i+1)*ntrials+0.5),'grey')
...
#conditions =((~empty_trials[1]&(~empty_trials[0]))&(with_sta_cells))
...
np.save('conditions_healthy',conditions)
#%%
...
#istim = 11 # , 1 for chirp
#plt.figure(figsize=(10,75))
#empty_ones = np.where(good_cells)[0][:]
#ntrials =10
...
#empty_ones = np.where((~((conditions))))[0][:]
#ntrials =10# Stimuli['SpikeTrains'][istim][0].shape[0]
#
#
#for i,e in enumerate(empty_ones):
#    for ii,st in enumerate(Stimuli['SpikeTrains'][istim][e][:]):
...
if LOAD_STORED_DISTANCES == False:
    ...
    np.save('SPIKE_dist_ys_5_rd10_neworder',SPIKE_dist_ys)
    np.save('ISI_dist_ys_5_rd10_neworder',ISI_dist_ys)
else:
    SPIKE_dist_ys = np.load('SPIKE_dist_ys_5_rd10_neworder.npy')
    ISI_dist_ys   = np.load('ISI_dist_ys_5_rd10_neworder.npy')
...
if SAVE_FIGS:
    plt.savefig('clusters_comparison_mixed_5.pdf', bbox_inches='tight')    
...
if SAVE_FIGS:
    plt.savefig('clusters_distance_matrix_clustered_SPIKE_mixed_5_'+st_name+'.pdf', bbox_inches='tight')    
...
plot_widths = [1.2,.8,5,1.8,0.5,2, .5]
...
if SAVE_FIGS:
    plt.savefig('clusters_summary_chirp_color_SPIKE_mixed_5_'+st_name+'.pdf', bbox_inches='tight')
...
#stim=0
psths = [] 
for sts in Stimuli['SpikeTrains'][stim][conditions_all]:
...
if SAVE_FIGS:
    plt.savefig('clusters_tsne_chirp_SPIKE_6_'+st_name+'.pdf', bbox_inches='tight')
...
zssta_dstance_flat=zssta_dstance[np.triu_indices(157, k = 1)]
...
plt.ylabel('Gap statistic');
if SAVE_FIGS:
    plt.savefig('Gap statistic_esta_healthy.pdf', bbox_inches='tight')
...
s = plt.scatter(X_r[:,0],X_r[:,1],s=16,lw=2,c=show_order_[fcls-1],
                cmap=ListedColormap(sns.hls_palette(n_flat_clusters_,l=0.6,s=0.6).as_hex()))
cb = plt.colorbar(s)
cb.set_label('Cluster')
...
axes.scatter(X_r[:,0], X_r[:,1],s=16,lw=0, c=show_order_[fcls-1],
             cmap=ListedColormap(sns.hls_palette(n_flat_clusters_,l=0.6,s=0.6).as_hex()))
axes.set_xlabel('PC1')
axes.set_ylabel('PC2')
...
axes.scatter(X_r[:,0], X_r[:,2],s=16,lw=0, c=show_order_[fcls-1],
             cmap=ListedColormap(sns.hls_palette(n_flat_clusters_,l=0.6,s=0.6).as_hex()))
axes.set_xlabel('PC1')
axes.set_ylabel('PC3')
...
#plt.ylim([-10,10])
if SAVE_FIGS:
    plt.savefig('Nornalized_PCA_projection_STA'+st_name+'.pdf', bbox_inches='tight')
...
unqnames = np.unique(Fnames[conditions_all][np.where(fcls == c+1)])
...
if SAVE_FIGS:
    plt.savefig('clusters_tsne_normalized_esta.pdf', bbox_inches='tight')
            #%%
