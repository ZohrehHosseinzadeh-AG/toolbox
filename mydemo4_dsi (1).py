...
#        O = spikeclass(sf)
        ...
        try:
            ...
            esta[i]= ESTA#['e_sta']
            esta_all=np.hstack((esta_all,esta[i]['e_sta']))
...
empty_trials[1] = np.asarray([np.mean([len(s) for s in st])<MIN_SPIKES_CHIRP for st in Stimuli['SpikeTrains'][1]])
...
for cel in range(ncl):# CELL NUMBER
    ...
    for s in range(len(st)):# TRIAL NUMBER
        ...
        if len(st[s])>0:
            ...
            psthtrl.append(psth_trl(st[s],250))
...
good_cells=(var_idx<.59)#&( var_idx>0.6)
good_cells_idx = np.where(((~empty_trials[1])&(good_cells)))[0][:]
bad_cells_idx = np.where(((~good_cells)|(empty_trials[1])))[0][:]
...
#    plt.plot((0,32000),((i+1)*ntrials+0.5,(i+1)*ntrials+0.5),'grey')
plt.title('Valid units')

plt.figure(figsize=(12,105))
ntrials =10
