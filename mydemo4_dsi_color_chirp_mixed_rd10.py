...
report_filenames  = [ data_path+"2020_02_07_rd10_l1_before_report.txt",
                     data_path+"2020_02_07_rd10_l2_report.txt",
...
stim_durs=np.array([4,12,16,4,4,4,4,4,4,4,12]) *Sampling   
SpikeTimes = {}
...
sts_chirp = Stimuli['SpikeTrains'][0]#[conditions]
...
for i,e in enumerate(good_cells_idx):
    ...
    plt.plot((0,16000),((i+1)*ntrials+0.5,(i+1)*ntrials+0.5),'grey')
...
if LOAD_STORED_DISTANCES == False:
    ...
    np.save('SPIKE_dist_ys_rd10_mixed',SPIKE_dist_ys)
    np.save('ISI_dist_ys_rd10_mixed',ISI_dist_ys)
else:
    SPIKE_dist_ys = np.load('SPIKE_dist_ys_rd10_mixed.npy')
    ISI_dist_ys   = np.load('ISI_dist_ys_rd10_mixed.npy')
...
ax = plt.subplot(121)
s_labels = ('Full field','Color','Full field-Color')
...
if SAVE_FIGS:
    plt.savefig('clusters_comparison_mixed_rd10_before.pdf', bbox_inches='tight')    
...
if SAVE_FIGS:
    plt.savefig('clusters_distance_matrix_clustered_SPIKE_mixed_rd10_beforr.pdf', bbox_inches='tight')    
...
if SAVE_FIGS:
    plt.savefig('clusters_summary_chirp_color_SPIKE_rd10_before.pdf', bbox_inches='tight')
...
if SAVE_FIGS:
    plt.savefig('clusters_tsne_chirp_SPIKE_rd10_before.pdf', bbox_inches='tight')
...
if SAVE_FIGS:
   plt.savefig('clusters_e_STA_color_mixed_rd10_before.pdf', bbox_inches='tight')
...
with open("clusetr_numbers_sta_rd10_beforr.txt", 'w') as output:
...
#        
#if SAVE_FIGS:
#    plt.savefig('clusters_e-STA_rd10.pdf', bbox_inches='tight')
#        