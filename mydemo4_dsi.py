...
#                                         spike_trains[i].spikes)
    ...
    return (vals)#PieceWiseConstFunc(edges, vals) 
...
for cel in range(ncl):# CELL NUMBER
    ...
    for s in range(len(st)):# TRIAL NUMBER
        ...
        if len(st[s])>0:
            ...
            psthtrl.append(psth_trl(st[s],200))
    
    var_idx[cel]=mean(np.var(psthtrl,axis=0,ddof=1))/var(psthtrl)
    FF[cel]=nanmean(np.std(psthtrl,axis=0,ddof=1)/np.mean(psthtrl,axis=0))
...
#%%
good_cells=(var_idx<.7)#&( var_idx>0.6)
