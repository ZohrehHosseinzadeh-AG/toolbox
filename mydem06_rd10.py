# -*- coding: utf-8 -*-
"""
Created on Sun Nov  8 15:10:05 2020
...
#            high_snr[i]=np.linalg.norm(peak_sta-mean(esta_i))>4*std(esta_ii)
 ...
 #           k2, p = stats.kstest(esta_i[:26], 'norm', args=(-800, 270))
#            from statsmodels.stats import weightstats as stests
 #           ztest ,p = stests.ztest(esta_i[:26],x2=esta_i[26:],value=mean(esta_i[26:]))
            
 #           acc=estimated_autocorrelation(esta_i[:26])
            freqs, psd = signal.welch(esta_i[:25],25, nperseg=50)
            freqs_base, psd_base = signal.welch(esta_i[25:],25, nperseg=50)
            psd_baseremoved=psd-psd_base
            psd_baseremoved=psd_baseremoved-psd_baseremoved.min()
            norm_psd=psd_baseremoved/sum(psd_baseremoved);
            fraction_psd=sum(norm_psd[(freqs>4.5)&(freqs<7.5)])
            fraction_psd2=sum(norm_psd[(freqs>1.5)&(freqs<4.7)])



            p_values[i]=(fraction_psd>.5)|(fraction_psd2>.5)|(p<.001)
...
for ii in range(len(goodstas)):
    if  not(goodstas[ii]==None):
        if len(goodstas[ii][2])>1:
            ...
            nsta=(rsta-rsta.min())
            gsta_matrix.append(nsta/np.max(nsta))
            
#gsta_matrix=gsta_matrix/np.max(np.abs(gsta_matrix), axis=1).reshape(-1,1)            
bsta_matrix=[]
bad_trl_sta=[]    
...
for ii in range(len(badstas)):
    if  not(badstas[ii]==None):
        if len(badstas[ii][2])>1:
            ...
            nsta=(rsta-rsta.min())
            bsta_matrix.append(nsta/np.max(nsta))
...
esta_time=np.asarray(goodstas[1][2].flatten())

plt.figure(figsize=(8,10))
plt.subplot(121)
           
plt.imshow(gsta_matrix,aspect='auto',extent=[-1,1,1,len(gsta_matrix)])
plt.ylabel('Unit #') 
plt.xlabel('Time s') 
plt.title('Significant STA') 
         
plt.subplot(122)
p=plt.imshow(bsta_matrix,aspect='auto',extent=[-1,1,1,len(bsta_matrix)])  
plt.xlabel('Time s') 
plt.title('Noisy STA') 
cb = plt.colorbar(p)
if SAVE_FIGS:
    plt.savefig('rd10_Good_VS_Bas_STA'+st_name+'.pdf', bbox_inches='tight')
    
    plt.savefig('rd10_Good_VS_Bas_STA'+st_name+'.png', bbox_inches='tight')
...
for figs in range(len(goodstas)):
    ...
    #esta_i=esta_i-mean(esta_i)
    esta_time=np.asarray(goodstas[figs][2].flatten())
    #
    ##from statsmodels.graphics import tsaplots
    ##fig = tsaplots.plot_acf(esta_i, lags=24)
...
#    freqs, psd = signal.welch(esta_i[:26],25, nperseg=50)
    ...
    #freqs, psd =signal.periodogram(esta_i,fs, 'flattop', scaling='spectrum')
    
#    fraction_psd=sum(norm_psd[(freqs>4)&(freqs<8)])
    
    plot(psd)
    plot(psd_base)
    plot(psd_baseremoved)
    ...
    for ps in range(len(freqs)):
        if sum(spsd)>=.5:
            ...
            #mask = np.ones(len(spsd), dtype=bool)
            #spsd=np.insert(spsd, np.argmax(spsd), 0)
            spsd=np.where(spsd==spsd[np.argmax(spsd)], 0, spsd) 
    #        mask[np.argmax(spsd)] = False
    #        spsd=spsd[mask]
    ...
    if SAVE_FIGS: 
        ...
        output_path=os.path.join('D:/Hame2/PSD_FIGS/'+paths)
...
#%%
#def estimated_autocorrelation(esta_i):
#    """
...
#    result = sum(r/(variance*(np.arange(n, 0, -1))))
...
#            st2.spikes=np.hstack((sts_chirp[cll][chc].spikes,sts_color[cll][chc].spikes+4000))# for second dataset flash stimulus has no data in the first 20 truals(+20)
...
#plot(spk.psth(sts_chirp[222][0:40],100).y)
#plot(spk.psth(ST2,100).y)
...
if LOAD_STORED_DISTANCES == False:
    ...
    np.save('SPIKE_dist_ys_rd10_f',SPIKE_dist_ys)
    np.save('ISI_dist_ys_rd10_f',ISI_dist_ys)
else:
    SPIKE_dist_ys = np.load('SPIKE_dist_ys_rd10.npy')
    ISI_dist_ys   = np.load('ISI_dist_ys_rd10.npy')
...
plot_clustrs(flash_stimuli,fcls,OOi,conditions_all,esta_all_rd_after,Cell_names_rd_after,plot_stims,'rd10_After')
...
plot_clustrs(flash_stimuli_before,fcls,OOi,conditions_all,esta_all,Cell_names,plot_stims,'rd10_Before')
...
#                             for i in range(len(sizes))])
        plt.pie(rcrding_numbers, shadow=True, startangle=30)
...
if SAVE_FIGS:
    plt.savefig('clusters_tsne_normalized_esta_rd10.pdf', bbox_inches='tight')
...
t=cluster_number
plot_stims=[0,1]       
eata_clustsering(Stimuli,fcls_sta,conditions_all,esta_all,Cell_names,plot_stims,t,'rd10_Before1')

 
t=cluster_number
plot_stims=[0]       
eata_clustsering(flash_stimuli,fcls_sta,conditions_all,esta_all_rd_after,Cell_names_rd_after,plot_stims,t,'rd10_After1')
...
#plt.ylim([-10,10])
if SAVE_FIGS:
    plt.savefig('Nornalized_PCA_projection_STA_rd10'+st_name+'.pdf', bbox_inches='tight')