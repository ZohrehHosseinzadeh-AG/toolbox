...
for ix,(rf,tf) in enumerate(zip(report_filenames,trigger_filenames)):
    ...
    trg[ix] = loadmat(tf)# trigges of each dataset[ix]
...
for ix in range(len(ClusterIDs)):
  ...
  for stimid in Stimuli[ix].index:
    ...
    n_trials    = Stimuli[ix]['NTrials'][stimid]# NUMBER OF TRIALS OF EACH STIMULUS
...
#    stim_start_end = get_stimtimes(stimid, Stimuli[ix], timeStampMatrices[ix]).reshape([n_trials,-1])
    if stimid==0:        
        ...
        stim_start_end=np.array(trg[ix].get('chirp2')).flatten().reshape([n_trials,-1])*Sampling# ONSET OF TRIALS
    elif stimid==2:
...
#  SpikeTimes[ix] = []
#  for cl in range(ncl):
#    cl_spikes = np.where(ClusterIDs==cl)[0]
#    cl_times  = np.unique(Times[cl_spikes])
#    SpikeTimes[ix].append(cl_times)
##      SpikeTimes[ix]=Times*50000
##SpikeTimes[ix+1].append(Times)
#
#for ix in range(2):
#  STs = []
#  for stimid in Stimuli[ix].index:
...
#    stim_img_n  = Stimuli[ix]['Nstim1'][stimid] / n_trials
#    stim_img_ms = Stimuli[ix]['Nrefresh'][stimid] * (1000/60)
#    stim_dur    = 4*50000#np.ceil(stim_img_ms * (Sampling/1000) * stim_img_n)
#    # get the stimulus start times and reshape to [n_trials,-1]
#    stim_start_end = get_stimtimes(stimid, Stimuli[ix], timeStampMatrices[ix]).reshape([n_trials,-1])
#
#    for cl in range(ncl):
...
valid_units = np.ones(ncl[0], dtype=bool)
...
empty_ones = np.where(((~empty_trials[0])&(~empty_trials[1])))[0][:]
...
t = t_best_chirp
a=SPIKE_dist_ys_valid[0]
b=SPIKE_dist_ys_valid[1]
